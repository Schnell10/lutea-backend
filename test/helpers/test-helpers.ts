...
/**
 * Je crée un utilisateur de test VÉRIFIÉ
 */
export async function createTestUser(app: INestApplication, userData?: Partial<any>) {
  ...
  // Si c'est un admin, je dois le créer directement dans la DB
  ...
  // Pour les utilisateurs normaux : j'utilise le flow register + validation

  // Étape 1 : Je crée le compte temporaire (sans le champ 'role' s'il existe)
  const { role: _role, ...registerData } = defaultUser as any;
  await request(app.getHttpServer())
    .post('/auth/register')
    .send(registerData);

  // Étape 2 : Je récupère le token de validation depuis la DB
  ...
  // Étape 3 : Je valide l'email avec le token
...
/**
 * J'extrais les cookies depuis les headers
 */
function extractCookiesFromHeaders(headers: any): string {
  const setCookies = headers['set-cookie'] || [];
  const cookiesArray = Array.isArray(setCookies) ? setCookies : [setCookies];
  
  // J'extrais uniquement la partie "name=value" de chaque cookie
  const cookieParts = cookiesArray.map((cookie: string) => {
    const match = cookie.match(/^([^=]+=[^;]+)/);
    return match ? match[1] : cookie;
  });
  
  // Je joins les cookies avec "; " pour le format Cookie header
  return cookieParts.join('; ');
}

/**
 * Je me connecte et je récupère les tokens
 * Je gère automatiquement la 2FA pour les admins
 */
export async function loginTestUser(app: INestApplication, email: string, password: string) {
  ...
  if (loginResponse.body.requires2FA) {
    // Je récupère le code 2FA depuis la DB
    const UserModel = app.get(getModelToken('User'));
    ...
    // Je soumets le code 2FA pour finaliser la connexion
...
/**
 * Je fais une requête authentifiée
 */
export async function authenticatedRequest(
  ...
  // J'ajoute les cookies d'authentification
  if (cookies) {
    req.set('Cookie', cookies);
  }
  
  // J'ajoute le body si présent
  if (body) {
    req.send(body);
  }
...
export async function createTestRetreat(app: INestApplication, authToken: string, retreatData?: Partial<any>) {
  ...
  // J'extrais prix, places, adresseRdv de retreatData s'ils existent pour les mettre dans dates
  ...
  // J'envoie la requête avec les cookies d'authentification
...
export async function createTestBooking(
  ...
  // Je récupère la retraite pour utiliser ses dates exactes
  ...
  // J'utilise la première date de la retraite (les dates doivent correspondre exactement)
  const retreatDate = retreat.dates[0];
  
  // J'utilise exactement comme dans le frontend : selectedBlock?.start et selectedBlock?.end
  ...
  // J'utilise directement les valeurs sans conversion pour correspondre exactement
  const dateStart = retreatDate.start;
  const dateEnd = retreatDate.end;
  
  const defaultBooking = {
    retreatId: retreatId, // Je m'assure que retreatId est bien présent
    ...
    dateStart: dateStart, // J'utilise directement comme dans le frontend (ligne 359 de booking.jsx)
    dateEnd: dateEnd, // J'utilise directement comme dans le frontend (ligne 360 de booking.jsx)
    nbPlaces: 1,
    participants: [
      {
        prenom: 'Test',
...
/**
 * Je nettoie la base de données de test
 * Je supprime toutes les collections créées pendant les tests
 */
export async function cleanupTestDatabase(app: INestApplication): Promise<void> {
  try {
    // Je récupère la connexion MongoDB depuis l'application
    ...
    for (const collectionName of collections) {
      try {
        ...
        // Si la collection n'existe pas, je continue
    ...
    // Je ne fais pas échouer les tests si le nettoyage échoue
  }
}

/**
 * J'attends un certain temps (pour les tests de timeout)
 */
